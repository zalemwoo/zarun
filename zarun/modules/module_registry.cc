...
Local<FunctionTemplate> GetDefineTemplate(Isolate* isolate) {
  ...
  Local<FunctionTemplate> templ = data->GetFunctionTemplate(&g_wrapper_info);
  if (templ.IsEmpty()) {
...
void ModuleRegistry::RegisterGlobals(Isolate* isolate,
  ...
  templ->Set(gin::StringToSymbol(isolate, "define"),
...
void ModuleRegistry::InstallGlobals(v8::Isolate* isolate,
  ...
  obj->Set(gin::StringToSymbol(isolate, "define"),
...
void ModuleRegistry::Load(Isolate* isolate, scoped_ptr<PendingModule> pending) {
  ...
  if (gin::ConvertFromV8(isolate, module, &factory)) {
    gin::PerContextData* data =
        gin::PerContextData::From(isolate->GetCurrentContext());
    gin::Runner* runner = data->runner();
    module = runner->Call(factory, runner->global(), argc,
                          argv.empty() ? NULL : &argv.front());
    if (pending->id.empty())
      gin::ConvertFromV8(isolate, factory->GetScriptOrigin().ResourceName(),
                         &pending->id);
  }
